...
class Player(Ship):
    ...
    def healthbar(self, window):
        ...
        ) + 10, round(self.ship_img.get_width() * (self.health/self.max_health)), 10))
...
def main():
    ...
    while run:
        ...
        if keys[pygame.K_LEFT] and player.x - player_vel > 0:  # links
            player.x -= player_vel
        if keys[pygame.K_RIGHT] and player.x + player_vel + player.get_width() < WIDTH:  # rechts
            player.x += player_vel
        if keys[pygame.K_UP] and player.y - player_vel > 0:  # hoch
            player.y -= player_vel
        if keys[pygame.K_DOWN] and player.y + player_vel + player.get_height() < HEIGHT:  # runter
            player.y += player_vel
